...
namespace YamlConfigs{
    void YamlLoad(std::shared_ptr<CameraData::CameraConfigParameters> camera_config_parameters, std::shared_ptr<MapData::MapConfigParameters> map_config_parameters, std::shared_ptr<CameraData::CameraInsideParameters> camera_inside_parameters, std::shared_ptr<CameraSpace::CameraPhysicalPose> camera_physical_pose)
    {       
       ...
       {
            auto camera_position_node=global_config["position"];
            camera_physical_pose->orientation=Eigen::Vector3d(
                camera_position_node["x"].as<double>(),
                camera_position_node["y"].as<double>(),
                camera_position_node["z"].as<double>()
            );
            auto camera_orientation_node=global_config["orientation"];
            camera_physical_pose->position=Eigen::Vector3d(
                camera_orientation_node["roll"].as<double>(),
                camera_orientation_node["pitch"].as<double>(),
                camera_orientation_node["yaw"].as<double>()
            );
       }
       else
       {
