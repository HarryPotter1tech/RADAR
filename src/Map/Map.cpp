...
bool Map::Mapload() {
    ...
    const bool loaded = tinyobj::LoadObj(
        &attrib_, &shapes_, &materials_, &warn_, &err_, Mapconfig_->obj_map_file_path.c_str(),
        nullptr, true, false);
...
std::vector<Eigen::Vector3d> Map::Getvector() const {
    ...
    for (size_t i = 0; i < attrib_.vertices.size(); i += 3) {
        ...
        std::cout << std::fixed << std::setprecision(3) << "Vertex " << i / 3 << ": (" << x << ", "
                  << y << ", " << z << ")" << std::endl;
    }
    return Vertices;
}
std::vector<Eigen::Matrix3d> Map::GetTriangle() const {
    ...
    for (size_t i = 0; i < shapes_.size(); i++) {
        for (size_t j = 0; j < shapes_[i].mesh.indices.size() / 3; j++) {
            ...
            std::cout << std::fixed << std::setprecision(3) << "Triangle " << j << ": "
                      << std::endl;
